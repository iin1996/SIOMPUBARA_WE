...
if (empty($_SESSION['namaadmin']) AND empty($_SESSION['leveladmin'])){
	...
	elseif ($module==$module2 AND $act=='add'){
				...
				try{
					$stmt = $pdo->prepare("INSERT INTO mutasi
											(id_dusun,nik,asal_dusun,tujuan_mutasi,tanggal_mutasi,keterangan)
											VALUES(:id_dusun,:nik,:asal_dusun,:tujuan_mutasi,:tanggal_mutasi,:keterangan)" );
					
					$stmt->bindParam(":id_dusun", $_POST["id_dusun"], PDO::PARAM_STR);
					$stmt->bindParam(":nik", $_POST["nik"], PDO::PARAM_STR);
					$stmt->bindParam(":asal_dusun", $_POST["asal_dusun"], PDO::PARAM_STR);
					$stmt->bindParam(":tujuan_mutasi", $_POST["tujuan_mutasi"], PDO::PARAM_STR);
					$stmt->bindParam(":tanggal_mutasi", $_POST["tanggal_mutasi"], PDO::PARAM_STR);
					...
					$sql = "UPDATE penduduk
							SET id_dusun 			= :id_dusunn
							WHERE nik 	= :nikk
						  ";
						  
					$statement = $pdo->prepare($sql);
					$statement->bindParam(":id_dusunn", $_POST["tujuan_mutasi"], PDO::PARAM_STR);
					$statement->bindParam(":nikk", $_POST["nik"], PDO::PARAM_STR);
	...
	elseif ($module==$module2 AND $act=='remove'){
		...
		$del = $pdo->query("DELETE FROM mutasi WHERE id_mutasi ='$_GET[id]'");
		$del->execute();
